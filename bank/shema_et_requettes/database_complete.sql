...
CREATE EXTENSION IF NOT EXISTS "vector"; -- Pour les embeddings du chatbot IA
...
-- Politiques simplifiées pour user_roles
CREATE POLICY "Users can view own roles" ON public.user_roles
FOR SELECT USING (user_id = auth.uid());

-- Fonction pour récupérer les rôles de l'utilisateur actuel
...
-- =============================================
-- 34. SYSTÈME DE RÉVISION DES ACTIVITÉS
-- =============================================

-- Table des commentaires de révision sur les activités non validées
...
CREATE INDEX idx_revision_comments_activity ON public.revision_comments(activity_id);
CREATE INDEX idx_revision_comments_created_by ON public.revision_comments(created_by);
CREATE INDEX idx_revision_comments_created_at ON public.revision_comments(created_at DESC);
...
CREATE TABLE public.revisionniste_activity_views (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    activity_id UUID NOT NULL REFERENCES public.activities(id) ON DELETE CASCADE,
...
-- Index pour optimiser les requêtes
CREATE INDEX idx_revisionniste_views_activity ON public.revisionniste_activity_views(activity_id);
CREATE INDEX idx_revisionniste_views_user ON public.revisionniste_activity_views(revisionniste_id);
CREATE INDEX idx_revisionniste_views_last_viewed ON public.revisionniste_activity_views(last_viewed_at DESC);
...
CREATE INDEX idx_activity_ratings_revisionniste ON public.activity_ratings(revisionniste_id);
CREATE INDEX idx_activity_ratings_rating ON public.activity_ratings(rating DESC);
...
-- Fonction pour calculer la note moyenne d'une activité
...
-- Fonction pour enregistrer/mettre à jour une vue d'activité
...
-- =============================================
-- 35. CONFIGURATION DES BUCKETS DE STOCKAGE (STORAGE)
-- =============================================
...
-- =============================================
-- 36. CHATBOT IA - ASSISTANCE À LA NÉGOCIATION
-- =============================================

-- Types ENUM pour le chatbot
...
-- Table des embeddings de documents (RAG)
...
-- Fonction de vérification d'accès au chatbot
...
-- Trigger pour générer le titre de session
...
-- Trigger pour mettre à jour l'activité de session
...
-- Commentaires de documentation
COMMENT ON TABLE public.ai_chat_sessions IS 'Sessions de conversation avec le chatbot IA pour l''assistance à la négociation';
COMMENT ON TABLE public.ai_chat_messages IS 'Messages échangés dans les sessions de chat avec références aux documents sources';
COMMENT ON TABLE public.document_embeddings IS 'Embeddings vectoriels des documents pour la recherche sémantique (RAG)';
COMMENT ON TABLE public.ai_chat_feedback IS 'Feedbacks des utilisateurs sur les réponses du chatbot';
