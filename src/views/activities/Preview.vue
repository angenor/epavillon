<template>
  <div class="min-h-screen relative transition-colors duration-200">
    ...
    <div class="bg-yellow-500 text-black px-4 py-3 text-center font-medium relative z-20">
      <div class="max-w-4xl mx-auto flex items-center justify-center gap-2">
        <font-awesome-icon :icon="['fas', 'eye']" />
        <span>{{ t('activity.preview.banner.title') }}</span>
        <span class="hidden sm:inline">{{ t('activity.preview.banner.description') }}</span>
      </div>
    </div>
    ...
    <div v-else-if="error" class="flex items-center justify-center min-h-[60vh]">
      <div class="text-center">
        <font-awesome-icon :icon="['fas', 'exclamation-triangle']" class="text-red-500 text-4xl mb-4" />
        <h2 class="text-xl font-semibold text-gray-900 dark:text-white mb-2">{{ t('activity.preview.error.title') }}</h2>
        <p class="text-gray-600 dark:text-gray-400 mb-4">{{ t('activity.preview.error.message') }}</p>
        <router-link
          to="/events/dashboard"
    ...
    <div v-else>
      ...
      <div class="relative h-[40vh] sm:h-[50vh] min-h-[250px] sm:min-h-[300px] overflow-hidden">
        <!-- Image de couverture de l'activité -->
        ...
        <div class="relative h-full flex flex-col justify-end">
          <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8 sm:pb-12 md:pb-16">
            ...
            <div class="flex flex-wrap gap-2 sm:gap-3 mb-4 sm:mb-6">
              ...
              <span class="backdrop-blur-md bg-yellow-500/80 border border-yellow-400/50 text-black px-3 sm:px-4 py-1 sm:py-2 rounded-full text-xs sm:text-sm font-bold animate-fade-in-up animation-delay-300">
                <font-awesome-icon :icon="['fas', 'eye']" class="mr-2" />
                {{ t('activity.preview.badge') }}
              </span>
            </div>
...
const loadActivity = async () => {
  try {
    ...
    // Charger l'activité sans restriction de statut pour la prévisualisation
    ...
    // Vérifier que l'utilisateur connecté est bien celui qui a soumis l'activité
    if (!authStore.user || activityData.submitted_by !== authStore.user.id) {
      error.value = true
      return
    }
    ...
    console.error('Error loading activity for preview:', error)
    error.value = true
  } finally {
    loading.value = false
  }
